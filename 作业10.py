...
class BILLIARD(object):
    def __init__(self,_x0=sqrt(1),_y0=1,_vx0=1,_vy0=1,_dt=0.001,_time=500):
    ...
    def cal(self):            # use Euler method to solve billiard motion
        for i in range(self.n):
            ...
            if self.nextx**2+self.nexty**2>1:
                self.nextx=self.x[-1]
                self.nexty=self.y[-1]
                while not(self.nextx**2+self.nexty**2>1):
    ...
    def plot_phase(self,_ax,_secy=0):        # give phase-space plot
        ...
        _ax.plot(self.phase_x,self.phase_vx,'ob',markersize=1,label='Ellipse'+r'$a=2,b=1$')

# give a trajectory and phase space plot        
fig= plt.figure(figsize=(10,4))
ax1=plt.axes([0.1,0.15,0.35,0.7])
ax2=plt.axes([0.1,0.15,0.35,0.7])
ax1.set_xlim(-1.1,1.1)
ax1.set_ylim(-1.1,1.1)
ax2.set_xlim(-1.1,1.1)
ax2.set_ylim(-1.1,1.1)  
ax1.set_title('Elliptical boundary: '+r'$a^2 =1,b^2 = 1$',fontsize=18)
ax2.set_title('Phase-space: '+r'$a^2 =1,b^2 = 1$',fontsize=18)
